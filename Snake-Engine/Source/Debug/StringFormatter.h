...
{
  public:
    template <typename... TParameters> static std::string Combine(const std::string &message, TParameters... parameters)
    {
  ...
  private:
    ...
    static std::string ParametersTupleToString(const std::size_t index, TParameter parameter, TParameters... parameters)
    {
        if (index == 0)
        {
            std::ostringstream string_stream;
    ...
    static std::string ParametersTupleToString(std::size_t index)
    {
        return "";
    }
};
